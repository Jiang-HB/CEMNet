...
class TestData(Dataset):
    def __init__(self, path):
        ...
        self.n_points = 1024
        self.max_angle = 45 / 180 * np.pi
        self.max_trans = 1.0
        self.noisy = False
        self.subsampled = True
        self.num_subsampled_points = 768
    ...
    def __getitem__(self, index):
        ...
        pcd_src.points = open3d.utility.Vector3dVector(pcd1)
        open3d.geometry.estimate_normals(pcd_src)
        # print(np.asarray(pcd_src.points).shape)
        # print(np.asarray(pcd_src.normals).shape)
        # exit()
        pcd_tgt = open3d.geometry.PointCloud()
        pcd_tgt.points = open3d.utility.Vector3dVector(pcd2)
        open3d.geometry.estimate_normals(pcd_tgt)
        sample = {'idx': np.array(index, dtype=np.int32),'transform_gt':np.eye(4).astype('float32')}
        sample['points_src'] = np.concatenate([np.asarray(pcd_src.points),np.asarray(pcd_src.normals)],axis=1).astype('float32')
        sample['points_ref'] = np.concatenate([np.asarray(pcd_tgt.points),np.asarray(pcd_tgt.normals)],axis=1).astype('float32')
        sample['transform_gt'][:3, :3] = R_ab.astype('float32')
        sample['transform_gt'][:3, 3] = translation_ab.astype('float32')
        return sample
